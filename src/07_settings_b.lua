--[[
    Settings B — background image, configs, about, and BuildSettingsPanel.
]]
...
function LucidUI.Window:_buildBackgroundSettings()
    ...
    mkBtn("Auto Accent", self.Theme.Surface, function()
        ...
        LucidUI:Notify({ Title = "Detecting...", Message = "Sampling image pixels", Duration = 2 })
        local accent = self:AutoDetectAccent(url)
        if accent then
            self:SetAccent(accent)
            LucidUI:Notify({ Title = "Accent Detected", Message = "Color applied" })
        else
            LucidUI:Notify({ Title = "Detection Failed", Message = "This image can't be sampled", Accent = Color3.fromRGB(255,80,80) })
        end
    end)
...
function LucidUI.Window:AutoDetectAccent(url)
    ...
    if not AssetService or not AssetService.CreateEditableImageAsync then
        print("[LucidUI] AutoAccent: API not available")
        return nil
    end
    ...
    if not ok or not img then
        print("[LucidUI] AutoAccent: CreateEditableImageAsync failed")
        return nil
    end
    ...
    for _ = 1, 30 do
        ...
        if ok2 and s and s.X > 0 and s.Y > 0 then
            size = s
            break
        end
        task.wait(0.05)
    ...
    local stepX = math.max(1, math.floor(size.X / 25))
    local stepY = math.max(1, math.floor(size.Y / 25))
    ...
    if count == 0 then
        print("[LucidUI] AutoAccent: no pixels readable")
        return nil
    end

    local avg = Color3.new(rSum / count, gSum / count, bSum / count)
    local h, s, v = Color3.toHSV(avg)
    print("[LucidUI] AutoAccent:", h, s, v)
    return Color3.fromHSV(h, math.min(s * 1.5, 1), math.max(v, 0.7))
end
