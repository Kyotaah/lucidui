--[[
    ...
    Color picker modal:
      Pops in with a short Back-ease scale + fade (0.22s). Pops out
      with a quick 0.15s reverse. Header carries a Frames-built
      droplet icon — no emoji, no font glyph.
]]

-- ============================================================
-- Droplet icon — built from a rotated square + a circle.
-- The diamond's lower half is hidden behind the circle, leaving
-- a pointed tip at the top and a round body at the bottom.
-- ============================================================
local function BuildDropletIcon(parent, size, color)
    size = size or 18
    ...
    -- Pointed top (square rotated 45°). Its lower half sits inside
    -- the circle below, so only the top half is visible — that's
    -- the droplet tip.
    local diamondSide = size * 0.32
    ...
    -- Round body. Drawn after the diamond so it covers its lower half.
    local bodySize = size * 0.72
    local body = Create("Frame", {
        Size = UDim2.fromOffset(bodySize, bodySize),
...
-- ============================================================
-- Modern HSV color picker modal (with pop-in animation + droplet icon)
-- ============================================================
function LucidUI.Window:_openColorPicker(fieldKey, fieldLabel)
    ...
    -- Starts fully transparent; tweens to 0.55 for the fade-in.
    ...
    -- Pop scale — starts at 0.88, tweens to 1 with Back ease.
    local popScale = Instance.new("UIScale")
    popScale.Scale = 0.88
    popScale.Parent = card

    -- ── Header: droplet icon + label ───────────────────────────
    local iconHolder = BuildDropletIcon(card, 18, self.Theme.Accent)
    iconHolder.Position = UDim2.fromOffset(18, 16)
    iconHolder.ZIndex = 502
    ...
    Create("TextLabel", {
        ...
        TextColor3 = self.Theme.TextPrimary, BackgroundTransparency = 1,
        TextXAlignment = Enum.TextXAlignment.Left,
        Position = UDim2.fromOffset(44, 14),
    ...
    -- ── Bottom row: preview, hex input, Done ───────────────────
    ...
    -- ── Pop-in animation ───────────────────────────────────────
    -- Fade the backdrop, pop the card with Back ease, fade contents.
    -- Total time ~0.25s. Snappy, not slow.
    TweenService:Create(modal, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
